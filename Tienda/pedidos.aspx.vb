...
Public Class pedidos
    ...
    Private Sub pedidos_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ...
        Dim Contador2a As Integer = 0
        Dim table2a As New DataTable
        Dim ObjetoAdapador2a As New DataSet1TableAdapters.carritoTableAdapter
        table2a = ObjetoAdapador2a.ConsultaPrePedidosUser(id_user_number)

        Dim ResultadoFinal As Object
        Dim obj(table2a.Rows.Count - 1, 5) As Object

        While (Contador2a < table2a.Rows.Count)
            Dim row2a As DataRow = table2a.Rows(Contador2a)
            Dim Contador2 As Integer = 0
            Dim table2 As New DataTable
            Dim ObjetoAdapador2 As New DataSet1TableAdapters.carritoTableAdapter
            Dim tem As String = row2a.Item("num_pedido")
            table2 = ObjetoAdapador2.ConsultaPedidosUser(id_user_number, row2a.Item("num_pedido"))



            While (Contador2 < table2.Rows.Count)
                Dim row2 As DataRow = table2.Rows(Contador2)
                obj(Contador2a, 0) = row2.Item("Estado_Compra")
                obj(Contador2a, 1) = row2.Item("Numero_Pedido")
                obj(Contador2a, 2) = row2.Item("Fecha_del_pedido")
                Select Case row2.Item("Precio_del_envio")
                    Case 180
                        obj(Contador2a, 3) = "Entrega del Dia Siguiente"
                    Case 105
                        obj(Contador2a, 3) = "Entrega Estandar"
                    Case 0
                        obj(Contador2a, 3) = "Recoleccion Personal"
                End Select
                obj(Contador2a, 4) = row2.Item("Precio_del_envio")
                obj(Contador2a, 5) = row2.Item("Cantidad_del_pedido")
